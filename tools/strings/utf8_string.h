...
struct utf8_string {
    ...
    utf8_string(std::vector<value_type>&& src) : data_m(std::move(src)) {}
    //explicit utf8_string(const char* str) : data_m(str, str + strlen(str)) {}

    explicit utf8_string(std::initializer_list<value_type> il) : data_m(il) {}
    explicit utf8_string(std::initializer_list<char> il) : data_m(reinterpret_cast<std::initializer_list<value_type>&>(il)) {}

    template<size_t N>
    explicit utf8_string(const char (&str)[N]) : data_m(str, str + N-1) {}
