...
struct utf8_view {
    ...
    constexpr utf8_view() noexcept : utf8_view(nullptr, nullptr) {}
    ...
    explicit utf8_view(const utf8_string& s) noexcept : utf8_view(s.data(), s.data() + s.byte_count().v) {}
    ...
    constexpr explicit utf8_view(const char (&str)[N]) noexcept
        : utf8_view(reinterpret_cast<const value_type*>(str),
                    reinterpret_cast<const value_type*>(str + N-1)) {}

    // enable optional
    // equal if view on the same range
    // use content_equals for content comparison
    constexpr bool operator==(const utf8_view& o) const {
        return start_m == o.start_m && end_m == o.end_m;
    }

    constexpr bool content_equals(const utf8_view& o) const {
        return byte_count().v == o.byte_count().v
                && meta::equals(*this, o.begin());
    }
    ...
    optional_code_point_t pull_code_point() {
        ...
        return pop().map([=](value_type c0) -> optional_code_point_t {
            ...
            if ((c0 & 0xC0) == 0x80) return {}; // only valid for continuations
            if ((c0 & 0xE0) == 0xC0) {
                return pop().map([=](value_type c1) -> optional_code_point_t {
                    if ((c1 & 0xC0) != 0x80) return {};
                    return code_point_t{((c0 & 0x1Fu)<<6)
                                | ((c1 & 0x3Fu) << 0)};
                });
            }
            if ((c0 & 0xF0) == 0xE0) {
                return pop().map([=](value_type c1) -> optional_code_point_t {
                    ...
                    return pop().map([=](value_type c2) -> optional_code_point_t {
                        ...
                        return code_point_t{((c0 & 0x0Fu)<<12)
                                    | ((c1 & 0x3Fu) << 6)
                                    | ((c2 & 0x3Fu) << 0)};
                    });
                });
                ...
                return pop().map([=](value_type c1) -> optional_code_point_t {
                    ...
                    return pop().map([=](value_type c2) -> optional_code_point_t {
                        ...
                        return pop().map([=](value_type c3) -> optional_code_point_t {
                            ...
                            return code_point_t{ ((c0 & 0x07u)<<18)
                                        | ((c1 & 0x3Fu) << 12)
                                        | ((c2 & 0x3Fu) << 6)
                                        | ((c3 & 0x3Fu) << 0)};
                        });
...
private:
    ...
    template<size_t N>
    bool pop() {
        if (N > byte_count().v) { start_m = end_m; return false; }
        start_m += N;
        return true;
    }
