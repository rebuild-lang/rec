...
type `vec(E:Type)`::
  ...
  ** `.at(index:u64) -> :*E`
  ** `.append(*:Type) -> :vec`
  ** `.replace(index:u64, :Type) -> :vec`
  ** `.remove(index:u64) -> :vec`
...
type `Expression`::
  ** = `variant(Void, Literal, NamedTuple, Invocation, Reference, Block)`
...
type `ArgumentAssignment`::
  ** ! no parser (only part of invocation)
  ** `.argument: *instance.Argument`
  ** `.values: vec(Expression)` _a value for each splat instance_
...
type `LeftArguments`::
  ...
  ** `.arguments : vec(instance.Argument)`
...
type `Function`::
  ...
  ** `.leftArguments :vec(*Argument)`
  ** `.rightArguments :vec(*Argument)`
  ** `.results :vec(*Argument)`
  ** _TODO_ `.precedencLevel :*Level = ()`
...
`caller : Context`::
  ** context of the current function invocation
