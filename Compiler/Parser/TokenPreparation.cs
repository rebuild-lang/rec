...
namespace REC.Parser
{
    public static class TokenPreparation
    {
        static bool IsRightSeparator(this Token token) {
            switch (token) {
            case Token.WhiteSpaceSeparator:
        ...
        static bool IsLeftSeparator(this Token token) {
            switch (token) {
            case Token.WhiteSpaceSeparator: // [WS]tok
        ...
        public static IEnumerable<TokenData> Prepare(IEnumerable<TokenData> input) {
            using (var it = input.GetEnumerator()) {
                do {
                    ...
                    || it.Current.Type == Token.WhiteSpaceSeparator); // skip to valid token
                ...
                while (it.MoveNext()) {
                    ...
                    if (previous.Type == Token.ColonSeparator) {
                        ...
                        while (current.Type == Token.WhiteSpaceSeparator
